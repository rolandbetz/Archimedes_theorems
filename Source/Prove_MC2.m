function [ ] = Prove_MC2(n, delta)
% n is the number of points you want to generate, delta is a parameter
% which indicates how close to the surface you want to count the generated
% points, or in other words how close should be a point to the surface to
% be acknowledged as a "surface point"

container_matrix=zeros(3,n); % a matrix to hold all the coordinates generated

% if a point will be accepted as "surface point", we will count them by
% theese paramters below
cube_surface=0;
sphere_surface=0;

% Below we generate the random numbers. The center of the generated points
% this time will be at coordinates (0,0,0), and all coordinates have a
% range of [-(1+delta), 1+delta]
for i=1:n
    container_matrix(1,i)=2*(1+delta)*rand()-(1+delta);
    container_matrix(2,i)=2*(1+delta)*rand()-(1+delta);
    container_matrix(3,i)=2*(1+delta)*rand()-(1+delta);
    
    if abs(container_matrix(1,i))>=1-delta || abs(container_matrix(2,i))>=1-delta || abs(container_matrix(3,i))>=1-delta
        cube_surface=cube_surface+1;
        % "counting" cube surface
    end
    if sqrt((container_matrix(1,i))^2+(container_matrix(2,i))^2+(container_matrix(3,i))^2)>=1-delta && sqrt((container_matrix(1,i))^2+(container_matrix(2,i))^2+(container_matrix(3,i))^2)<=1+delta
...
% The next 15 lines are for the plotting.
...
for i=1:n
    if abs(container_matrix(1,i))>=1-delta || abs(container_matrix(2,i))>=1-delta || abs(container_matrix(3,i))>=1-delta
        cube_matrix(:,cube_index)=container_matrix(:,i);
